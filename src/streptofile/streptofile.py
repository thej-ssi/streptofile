...
def extract_emm_type(emm_blast_tsv: Path):
    ...
    emm_typing_results = {"EMM_type": "-", "ENN_type": "-", "MRP_type": "-", "genes_in_operon": "-"}
    ...
    if blast_df_unique.height == 0:
        ...
        if blast_df_unique.height == 1:
            ...
            emm_typing_results["EMM_type"] = row0["qseqid"].lstrip("EMM")
            ...
            if blast_df_unique.height == 2:
                ...
                emm_typing_results["ENN_type"] = row1["qseqid"].lstrip("EMM")
                ...
                emm_maintype = row0["qseqid"].lstrip("EMM").split(".")[0]
                mrp_maintype = row1["qseqid"].lstrip("EMM").split(".")[0]

                if (
                    mrp_maintype in emm_types_in_emm_plus_mrp_operons
                    or emm_maintype in mrp_types_in_emm_plus_mrp_operons
                ):
                    emm_typing_results["MRP_type"] = emm_typing_results["EMM_type"].lstrip("EMM")
                    emm_typing_results["EMM_type"] = emm_typing_results["ENN_type"].lstrip("EMM")
                    emm_typing_results["ENN_type"] = "-"
                ...
                emm_typing_results["MRP_type"] = row0["qseqid"].lstrip("EMM")
                if row0["length"] < row0["qlen"] or row0["pident"] < 100:
                ...
                emm_typing_results["EMM_type"] = row1["qseqid"].lstrip("EMM")
                if row1["length"] < row1["qlen"] or row1["pident"] < 100:
                    emm_typing_results["EMM_type"] += "*"
                    notes.append(
                ...
                emm_typing_results["ENN_type"] = row2["qseqid"].lstrip("EMM")
                ...
                notes += details
        emm_typing_results["genes_in_operon"] = ",".join(blast_df_unique.get_column("qseqid").cast(pl.Utf8).to_list())
    else:
        emm_genes = []
    ...
    if len(notes) == 0:
        emm_typing_results["emm_typing_notes"] = "All exact allele matches"
    else:
...
def print_emm_results(emm_results_dicts: list[dict],
    ...
    emm_results_df = pl.from_dicts(emm_results_dicts)
...
def main():
    ...
    for assembly_file in args.input:
        ...
        emm_results_dicts.append(emm_results)
    output_file = args.output / "results.tsv"
    print_emm_results(emm_results_dicts=emm_results_dicts,
